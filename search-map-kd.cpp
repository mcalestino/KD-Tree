...
bool read_args(int argc, char **args, Params *params) {
    ...
    if (argc != 6) {
        std::cout << "Usage: ./search-map-kd <lat> <lon> <radius> <filter> <database>\n";
        ...
        std::cout << "\t./search-map-kd 42.34 -71.10 10 restaurant boston.txt\n";
        return false;
...
int main(int argc, char **args) {
    ...
    if (read_args(argc, args, &params)) {
        ...
        if (myfile.is_open()) {
            ...
            tic = std::clock();
            count = tree.printConvexHull(params.latitude, params.longitude, params.radius, params.filter);
            toc = std::clock();
